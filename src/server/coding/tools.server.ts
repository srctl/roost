...
export const codingTools: DynamicToolSpec[] = [
  ...
  spec(
    ...
    "Continue an existing ready/blocked coding job within its original assignment. If lastWorkerState is not_started, preparation failed before a worker launched: fix that dependency, then use this tool to retry the original launch. If its named Herdr server stopped, continuation can restart that server and verify the existing worker identity before sending input. A missing or changed worker still requires inspection; this does not create a replacement worker. Use a stable requestId to prevent duplicate prompts. A blocked approval cannot be answered by this tool: the user must resolve it in the worker's terminal first. Finish your reply after queueing; do not poll.",
    ContinueCodingJob,
  ),
  spec(
