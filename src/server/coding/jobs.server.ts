...
import { codingEnabled, requireCodingEnabled } from "./preference.server";
import {
  createCodingJob,
  getCodingSettings,
...
export function requireCodingRun(
  ...
  mode: "read" | "configure" | "start" | "continue" | "manage",
  jobId?: string,
) {
  if (mode !== "read" && mode !== "manage") requireCodingEnabled(db);
  if (
...
export const completeCodingJob = (
  ...
  withAgentStore((db) =>
    writeTransaction(db, () => {
      assertAvailable(db);
      requireCodingRun(db, agentId, runId, "manage", input.id);
      const job = readCodingJob(db, agentId, input.id);
      if (!job) throw new AgentStoreError({ message: "Coding job not found." });
...
export const codingInstructions = (agentId: string) =>
  Effect.gen(function* () {
    const settings = yield* getCodingSettings(agentId);
    const profiles = yield* listExecutionProfiles();
    if (!(yield* withAgentStore(codingEnabled)))
      return "Coding is disabled in Settings. Do not start or continue coding work. Existing jobs may be inspected, completed, or stopped when requested.";
