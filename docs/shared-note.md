...
Schema 14 integrates `agent_notes`, `note_revisions`, `note_requests`, and
`note_reads` with Threads and Jobs. The atomic upgrade checks table shapes to
support main schema 10, historical Notes-only schema 11, Threads schemas 11–13,
and combined databases without replacing saved data. Jobs retain their separate
feature-keyed migrations and active-report refusal preflight. Content, instruction, and restore
...
note to resolve a conflict. Tool version 15 refreshes existing persistent
