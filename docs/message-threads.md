...
Reply buttons create or reopen one thread per user/assistant message. Main stays
...
Media capture and publication are separate from this reusable harness.

Verification uses headless Linux Chromium at 1440×1000 and 390×844. The smaller
viewport exercises responsive layout and keyboard focus, not a physical phone.
Safari/WebKit, native mobile keyboards, real touch hardware and live model output
are not claimed as tested. The deterministic provider fixture exercises the real
runtime/tool/queue boundaries; its generated answers are simulations. Browser
storage availability is required for local draft/read-marker persistence.
