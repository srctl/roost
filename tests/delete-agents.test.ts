  ...
  fixture(async ({ directory, id, other, remove }) => {
    ...
    await store((db) => {
      for (const a of [id, other]) {
        ...
        db.prepare(
          "INSERT INTO runs(id,agentId,kind,prompt,status,createdAt) VALUES (?,?,'chat','Approval fixture','completed',0)",
        ).run(approvalRun, a);
        db.prepare(
          "INSERT INTO approvals VALUES (?,?,?,'thread','key','{}','pending',NULL,0,NULL)",
        ).run(randomUUID(), a, approvalRun);
        db.prepare(
...
for (const status of ["completed", "cancelled"] as const) {
  for (const inputStatus of ["queued", "dispatching"] as const) {
    test(`${status} job with ${inputStatus} direct input blocks deletion atomically`, async () =>
      fixture(async ({ id, remove }) => {
        const created = await job(id);
        await store((db) => {
          db.prepare("UPDATE coding_jobs SET status=? WHERE id=?").run(
            status,
            created.id,
          );
          db.prepare(
            "INSERT INTO coding_job_inputs(id,jobId,agentId,prompt,status,createdAt) VALUES('direct',?,?, 'Continue',?,0)",
          ).run(created.id, id, inputStatus);
          db.prepare(
            "INSERT INTO coding_worker_messages(inputId,jobId,agentId,sessionIdentity,nativeSessionId) VALUES('direct',?,?,'same','native')",
          ).run(created.id, id);
        });
        await assert.rejects(remove(), /queued or unsettled/);
        await store((db) => {
          assert.equal(
            db.prepare("SELECT count(*) n FROM deleted_agents").get()?.n,
            0,
          );
          assert.equal(
            db
              .prepare("SELECT status FROM coding_job_inputs WHERE id='direct'")
              .get()?.status,
            inputStatus,
          );
          db.prepare(
            "UPDATE coding_job_inputs SET status='abandoned' WHERE id='direct'",
          ).run();
        });
        await remove();
      }));
...
test("terminal job retains unsettled direct receipts and worker fences until explicitly settled", async () =>
  fixture(async ({ id, remove }) => {
    const created = await job(id);
    await store((db) => {
      ...
      db.prepare(
        "INSERT INTO coding_job_inputs(id,jobId,agentId,prompt,status,createdAt) VALUES('sent',?,?,'Continue','sent',0)",
      ).run(created.id, id);
      db.prepare(
        "INSERT INTO coding_worker_messages(inputId,jobId,agentId,sessionIdentity,nativeSessionId,deliveredAt,respondingAt) VALUES('sent',?,?,'same','native',1,2)",
      ).run(created.id, id);
      db.prepare(
        "INSERT INTO coding_worker_fences(inputId,jobId,agentId,sessionIdentity,nativeSessionId) VALUES('uncertain',?,?,'same','native')",
      ).run(created.id, id);
    });
    await assert.rejects(remove(), /queued or unsettled/);
    await store((db) =>
      db.exec("UPDATE coding_worker_messages SET completedAt=3"),
    );
    await assert.rejects(remove(), /queued or unsettled/);
    await store((db) =>
      db.exec("UPDATE coding_worker_fences SET acknowledgedAt=4"),
    );
    await remove();
  }));

test("combined deletion removes Notes/history, threads, job conversations and membership, fences all owned tables and preserves neighbors", async () =>
  fixture(async ({ id, other, remove }) => {
    const snapshots = await store((db) => {
      db.exec(
        "INSERT INTO agent_navigation_sections VALUES('section','Projects',0,0)",
      );
      for (const a of [id, other]) {
        db.prepare(
          "INSERT INTO agent_navigation_memberships VALUES(?,'section')",
        ).run(a);
        db.prepare(
          "INSERT INTO agent_notes VALUES(?,1,'[]','Keep notes',10)",
        ).run(a);
        db.prepare("INSERT INTO note_revisions VALUES(?,1,'{}','user')").run(a);
        db.prepare(
          "INSERT INTO note_requests VALUES(?,'request','fingerprint','{}')",
        ).run(a);
        db.prepare("INSERT INTO note_reads VALUES(?,?,'scope',1,100)").run(
          `token-${a}`,
          a,
        );
        db.prepare(
          "INSERT INTO conversation_records(id,agentId,parentConversationId,parentMessageId,createdAt) VALUES(?,?,?,'message',0)",
        ).run(`thread-${a}`, a, a);
        db.prepare(
          "INSERT INTO conversation_sessions(conversationId,agentId,threadId,archive) VALUES(?,?,?,'[]')",
        ).run(`thread-${a}`, a, `native-${a}`);
        db.prepare(
          "INSERT INTO provider_message_ids VALUES(?,?,?,'native-message','message')",
        ).run(a, `thread-${a}`, `native-${a}`);
        db.prepare(
          "INSERT INTO coding_job_workspaces VALUES(?,?,?,'{}',1,0)",
        ).run(`job-${a}`, a, `job-${a}`);
        db.prepare(
          "INSERT INTO coding_job_feedback VALUES(?,?,?,'revision',0,NULL)",
        ).run(`feedback-${a}`, `job-${a}`, a);
        db.prepare(
          "INSERT INTO coding_worker_messages(inputId,jobId,agentId,sessionIdentity,nativeSessionId,completedAt) VALUES(?,?,?,'same','native',1)",
        ).run(`input-${a}`, `job-${a}`, a);
        db.prepare(
          "INSERT INTO coding_worker_fences VALUES(?,?,?,'same','native',1)",
        ).run(`fence-${a}`, `job-${a}`, a);
      }
      // Discover ownership independently of the production deletion list.
      const tables = db
        .prepare(
        ...
        .filter((table) =>
          db
            ...
            .all()
            .some((column) => column.name === "agentId"),
        );
      return tables.map((table) => ({
        table,
        own: db.prepare(`SELECT * FROM ${table} WHERE agentId=?`).all(id),
        other: db.prepare(`SELECT * FROM ${table} WHERE agentId=?`).all(other),
      }));
    });
    await remove();
    await store((db) => {
      for (const snapshot of snapshots) {
        ...
        assert.deepEqual(
          db.prepare(`SELECT * FROM ${table} WHERE agentId=?`).all(id),
          [],
          table,
        );
        assert.deepEqual(
          db.prepare(`SELECT * FROM ${table} WHERE agentId=?`).all(other),
          snapshot.other,
          table,
        );
        for (const operation of ["INSERT", "UPDATE"])
          assert.ok(
            db
              ...
              .get(`deleted_${table}_${operation}`),
            `${table} ${operation} fence`,
          );
        for (const row of snapshot.own) {
        ...
        if (snapshot.other.length)
          assert.throws(
            () =>
              db
                .prepare(`UPDATE ${table} SET agentId=? WHERE agentId=?`)
                .run(id, other),
            /deleted/,
            `${table} late update`,
          );
...
test("uncertain failed direct submission blocks terminal-job deletion until inspection is acknowledged", async () =>
  fixture(async ({ id, remove }) => {
    const created = await job(id);
    await store((db) => {
      db.prepare("UPDATE coding_jobs SET status='cancelled' WHERE id=?").run(
        created.id,
      );
      db.prepare(
        "INSERT INTO coding_job_inputs(id,jobId,agentId,prompt,status,createdAt) VALUES('uncertain',?,?,'Continue','failed',0)",
      ).run(created.id, id);
      db.prepare(
        "INSERT INTO coding_worker_messages(inputId,jobId,agentId,sessionIdentity,nativeSessionId) VALUES('uncertain',?,?,'same','native')",
      ).run(created.id, id);
    });
    await assert.rejects(remove(), /queued or unsettled/);
    await store((db) =>
      db.exec(
        "UPDATE coding_job_inputs SET status='abandoned' WHERE id='uncertain'",
      ),
    );
    await remove();
  }));
