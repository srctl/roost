...
test("distinct conversations share agent Notes while read tokens and CAS remain run-owned", async () => {
  const directory = mkdtempSync("/tmp/roost-note-conversations-");
  const previous = process.env.ROOST_DATA_DIR;
  process.env.ROOST_DATA_DIR = directory;
  try {
    ...
    await run(
      withAgentStore((db) => {
        db.prepare(
          "INSERT INTO conversation_records(id,agentId,createdAt) VALUES(?,?,123)",
        ...
        for (const [id, conversationId] of [
          ...
          db.prepare(
            "INSERT INTO runs(id,agentId,conversationId,kind,prompt,status,createdAt) VALUES(?,?,?,'chat','Read Notes','queued',123)",
          ).run(id, agent.id, conversationId);
      }),
    );
    const paragraph = block("Shared content");
    await run(
      saveNote(agent.id, {
        requestId: randomUUID(),
        revision: 0,
    ...
    const main = await run(readNote(agent.id, "main-run"));
    const other = await run(readNote(agent.id, "sibling-run"));
    assert.deepEqual(main.blocks, other.blocks);
    const patch = {
      requestId: randomUUID(),
      revision: main.revision,
      readToken: main.readToken!,
      edits: [
        {
          id: paragraph.id,
          before: paragraph,
          after: { ...paragraph, content: [{ text: "Main edit" }] },
        },
      ],
    };
    await assert.rejects(
      run(patchNote(agent.id, "sibling-run", patch)),
      /Read the current/,
    );
    const saved = await run(patchNote(agent.id, "main-run", patch));
    assert.deepEqual(
      (await run(readNote(agent.id, "sibling-run"))).blocks,
      saved.blocks,
    );
    await assert.rejects(
      run(
        patchNote(agent.id, "sibling-run", {
          ...patch,
          requestId: randomUUID(),
    ...
    const fresh = await run(readNote(agent.id, "sibling-run"));
    const edit = {
      ...patch,
      requestId: randomUUID(),
      revision: fresh.revision,
      readToken: fresh.readToken!,
      edits: [{ id: paragraph.id, before: fresh.blocks[0]!, after: paragraph }],
    };
    for (const allowMutations of [false, "reflection"] as const) {
      const denied = await handleAgentTool(
        { agentId: agent.id, runId: "sibling-run", allowMutations },
        "roost_patch_note",
        edit,
      );
      assert.equal(denied.success, false);
    }
    await run(patchNote(agent.id, "sibling-run", edit));
    assert.deepEqual((await run(readNote(agent.id, "main-run"))).blocks, [
      paragraph,
    ]);
  } finally {
    if (previous === undefined) delete process.env.ROOST_DATA_DIR;
