                ...
                ForEach(model.sections()) { section in
                    Section {
                        ForEach(section.items) { item in
                            ...
                            .contextMenu { FeedItemActions(model: model, item: item) }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text(item.summary)
                ...
                .fixedSize(horizontal: false, vertical: true)
            if item.kind != "update", let raw = item.imageUrl, let url = workspaceURL(raw) {
                FeedStoryImage(url: url, title: item.title, identifier: "feed-row-image-" + item.id)
            }
            Text(Date(milliseconds: item.publishedAt), style: .time)
...
private struct FeedStoryImage: View {
    @Environment(\.palette) private var palette
    let url: URL
    let title: String
            ...
            Rectangle().fill(palette.surface)
                ...
                .overlay {
                    if let image = phase.image {
                        ...
                        Image(systemName: "photo").foregroundStyle(palette.faint)
                    } else {
                        ProgressView()
                    }
                }
                ...
                .accessibilityValue(
                    phase.image != nil
                        ? "Image preview"
                        : phase.error != nil ? "Image unavailable" : "Loading image"
                )
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                ...
                if current.kind != "update", let raw = current.imageUrl, let url = workspaceURL(raw)
                {
                    FeedStoryImage(
                        url: url, title: current.title,
                        identifier: "feed-reader-image-" + current.id)
                }
